...
def check_narration_with_gemini(narration_blocks, api_key):
    ...
    try:
        ...
        prompt = f"""
        あなたは、映像編集の最終チェックを行う、経験豊富なアシスタントディレクターです。
        あなたの主な任務は、ナレーターが読む原稿に「うっかりミス」がないかを発見することです。特に、編集中のコピー＆ペーストによる修正漏れには細心の注意を払ってください。

        # あなたの思考プロセス
        1. **ステップ1：連続性のチェック（最優先）**
           まず、各ナレーションブロックを、その**直前のブロック**と比較します。文章が大部分で共通しており、末尾や一部の単語だけが違う場合（例：「〇〇で独占配信！」と「〇〇で限定配信！」）、これは編集中のコピー＆ペーストミスである可能性が極めて高いと判断します。この場合は、最優先で指摘してください。

        2. **ステップ2：通常の誤字脱字チェック**
           次に、個々のブロック内で誤字脱字や固有名詞の間違いがないかを確認します。

        # その他の制約条件
        - ナレーション特有の句読点や文末のスペースは、意図的なものである可能性が高いため、修正しないでください。
        - 芸能人の名前は、誤りがあれば正しく校正してください。
        - 文末の表現が不自然に感じられても、意図的な演出（体言止めなど）の可能性があるため、無理に修正しないでください。
        - 数字は漢数字ではなく、算用数字のままにしてください。
        - 上記のいずれにも該当する問題がない場合は、必ず「問題ありませんでした。」とだけ出力してください。
        
        # 出力形式
        - 誤りがある場合のみ、以下のMarkdownテーブル形式で出力してください。
        - **類似性**を指摘する場合、「理由」の欄に「No.Xと文章が酷似しています。コピーミスや修正漏れの可能性があります。」のように、具体的に記載してください。
        
        【出力例】
        | No. | 修正提案 | 理由 |
        |---|---|---|
        | 8 | 本文全体を確認 | No.7と文章が酷似しています。コピーミスや修正漏れの可能性があります。 |
        | 12| 〇〇 → △△ | 誤字と思われます。 |
