...
def check_narration_with_gemini(narration_blocks, api_key):
    ...
    try:
        ...
        prompt = f"""
        あなたはプロの映像編集者であり、非常に注意深い校正者です。
        以下のナレーション原稿をチェックし、誤りや不自然な点があれば修正案を提示してください。

        # 特に注意すべき点
        - **直前のナレーションとの類似性チェック:** ナレーションの本文が、直前のブロックと「完全に同じ」または「酷似している（末尾だけ違うなど）」場合は、コピー＆ペーストのミスである可能性が非常に高いです。これは最重要のチェック項目として、必ず指摘してください。

        # その他の制約条件
        - ナレーション特有の句読点（、。）や文末のスペースは、意図的なものである可能性が高いため、修正しないでください。
        - 芸能人の名前や固有名詞は、誤りがあれば正しく校正してください。
        ...
        - 「No.」列には、必ず元のテキストの番号を入れてください。
        - 「修正提案」列では、誤字脱字は「○○ → △△」のように明確に記載してください。
        - 「理由」列は、「〇〇の誤り」や「〇〇と思われます」のように簡潔に記載してください。
        - **類似性**を指摘する場合、「理由」の欄に「No.Xと酷似」のように、どのナレーションと似ているかを必ず明記してください。
        
        【出力例】
        | No. | 修正提案 | 理由 |
        |---|---|---|
        | 5 | 本文全体を確認 | No.4と酷似しています。意図したものでない場合、修正が必要です。 |
        | 12| 〇〇 → △△ | 誤字と思われます。 |
