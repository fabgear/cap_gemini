...
def check_narration_with_gemini(narration_blocks, api_key):
    ...
    try:
        ...
        prompt = f"""
        あなたはプロフェッショナルなナレーション校正者です。
        ナレーションの誤字脱字をチェックしてください。
        ナレーション原稿なので句読点やスペースなどの読みやすい工夫があり正規文章とは違うことを理解して下さい
        
        
　　　　【校正ルール】
        1. 読みやすくするための工夫（過度な句読点、スペース）はそのままでＯＫ
        2. 芸能人の名前の漢字も校正
        3. 文末が途中で終わってる時も無視
        4. 漢数字は使用禁止です。漢数字に校正する必要はありません。
        
        
        【指示】
        1. 誤りが見つかった場合のみ、以下の Markdown テーブル形式で出力してください。
        2. 理由の部分は提案という形で書いてください。
        3. 「No.」の列には、必ず元の原稿の番号を入れてください。
        4. 誤りがない場合は、「問題ありませんでした。」とだけ出力してください。
...
【機能詳細】  
・ENDタイム(秒のみ)が自動で入ります。分をまたぐ時は(分秒)表記になり  
　次のナレーションと繋がる時は割愛されます  
・H（時間）をまたぐ時は自動で仕切りが入ります  
